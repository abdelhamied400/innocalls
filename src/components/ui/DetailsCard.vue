<template>
  ...
  >
    <div class="content grid grid-cols-5 flex-grow">
      <div class="border-r col-span-2 flex" :class="`bg-${stats}`">
        <div class="icon m-auto">
          <img class="w-20 h-20" :src="props.icon" alt="" />
        </div>
      </div>
      <div class="data col-span-3 my-auto px-4 py-8 flex flex-col gap-2">
        <h4
          class="title text-xl font-medium capitalize"
          :class="props.stats ? `text-${props.stats}` : 'text-primary'"
        >
          {{ props.title }}
        </h4>
        <h3
          class="value text-3xl font-medium capitalize"
          :class="props.stats ? `text-${props.stats}` : ''"
        >
